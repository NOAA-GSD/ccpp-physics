     subroutine gwdps_oro_v1(im,  km,    imx, do_tofd,           &
         ...
         prsi,del,prsl,prslk, zmeti, zmet, dtp, kdt, nmtvr,      &
         oro_stat,                 sgh30,                        &
...
!----------------------------------------      
      ...
      integer, intent(in) :: im, km, imx, kdt, nmtvr
      ...
      real(kind=kind_phys), intent(in) :: oro_stat(im,  nmtvr), sgh30(im)       
...
! locals SSO
!
      real(kind=kind_phys) :: oc(im), oa4(im,4), clx4(im,4)
      real(kind=kind_phys) :: hprime(im)
      real(kind=kind_phys) :: elvmaxd(im), theta(im)
...
!========================================================
      do i = 1,im      
          ...
          clx4(i,1)  = oro_stat(i,7)
          clx4(i,2)  = oro_stat(i,8)
          clx4(i,3)  = oro_stat(i,9)
          clx4(i,4)  = oro_stat(i,10)
          theta(i)  = oro_stat(i,11)
          gamma(i)  = oro_stat(i,12)
